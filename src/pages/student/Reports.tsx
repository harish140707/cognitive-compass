...
import { FileText, Download, TrendingUp, TrendingDown, Minus, Brain, AlertCircle } from "lucide-react";

export default function Reports() {
  const q = computeQuizStats();
  const reports = q.weeklyReports;
    ...
    <AppLayout>
      <div className="mb-6 flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Quiz-based cognitive analytics & improvement history</p>
        </div>
      ...
      {!q.hasData && (
        <div className="mb-6 flex items-center gap-2.5 bg-amber-400/10 border border-amber-400/20 rounded-lg px-4 py-2.5">
          ...
          <span className="text-xs text-amber-300">No quiz data yet — complete quizzes in <strong>Topic Mastery</strong> to generate reports.</span>
        </div>
      )}
      ...
      <div className="card-glow p-5 mb-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold font-display text-foreground text-sm">Topic Performance Timeline</h3>
        </div>
        {q.hasData ? (
            ...
            {reports.map((r, i) => (
              <div key={r.week} className="flex items-center gap-0 shrink-0">
                <div className="flex flex-col items-center">
                  ...
                  <div className="text-[9px] text-muted-foreground mt-1 w-16 text-center truncate">{r.week}</div>
                </div>
                {i < reports.length - 1 && <div className="w-8 h-px bg-border mx-1 mb-4" />}
              </div>
            ))}
          </div>
        ) : (
          <div className="py-6 text-center text-xs text-muted-foreground">Timeline will appear after quiz attempts</div>
        )}
      </div>
      ...
      <div className="card-glow overflow-hidden">
        <div className="px-5 py-4 border-b border-border flex items-center gap-2">
          ...
          <h3 className="font-semibold font-display text-foreground text-sm">Quiz Performance History</h3>
        </div>
        <div className="overflow-x-auto">
          {q.hasData ? (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  {["Topic", "Accuracy", "Attempts", "Improvement", "Cognitive Type", "Status", "Report"].map(h => (
            ...
            <div className="py-12 text-center text-xs text-muted-foreground">No quiz data yet</div>
          )}
        </div>
      </div>
      ...
      <div className="mt-4 card-glow p-5 border-l-2 border-primary">
        ...
        <p className="text-xs text-muted-foreground mb-3">Based on current quiz trajectory and behavioral patterns:</p>
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: "Projected Accuracy", value: `${Math.min(100, q.overallAccuracy + 9)}%`,  desc: q.hasData ? `+9% from current ${q.overallAccuracy}%` : "Take quizzes first", color: "text-teal" },
            { label: "Dropout Risk",       value: q.hasData ? (q.overallAccuracy < 50 ? "High" : q.overallAccuracy < 70 ? "Medium" : "Low") : "—", desc: q.hasData ? `${Math.max(5, 100 - q.overallAccuracy - 30)}% probability` : "Based on accuracy", color: q.hasData && q.overallAccuracy >= 70 ? "text-emerald-400" : "text-amber-400" },
            { label: "Mastery ETA",        value: q.weakTopics.length > 0 ? `${q.weakTopics.length * 1.5} wks` : "On track", desc: q.weakTopics.length > 0 ? q.weakTopics.slice(0, 2).join(" & ") : "No weak topics!", color: "text-violet" },
